...
def read_from_json(filepath, metric_dir=None):
    def find_matching_folder(metric_dir, save_name_suffix):
        for folder in os.listdir(metric_dir):
            if folder.endswith(save_name_suffix):
                return os.path.join(metric_dir, folder)
        raise FileNotFoundError(f"No folder ends with {save_name_suffix} in {metric_dir}")
    ...
    for record in data["_checkpoint"]["records"]:
        ...
        folder_path = find_matching_folder(metric_dir, record["save_name"])
        filepath = os.path.join(folder_path, 'metric_info.json')
        temp_dict = {}
