...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(self,
        ...
        outs_motion = self(bev_embed, track_query, lane_query, lane_query_pos, track_boxes, velocity, acceleration, yaw, command)
    ...
    def forward_test(self, bev_embed, velocity=None, acceleration=None, yaw=None, command=None, outs_track={}, outs_seg={}):
        """Test function"""
    ...
    def compute_matched_gt_traj(self,
        ...
        for i in range(num_imgs):
            ...
            if self.use_nonlinear_optimizer:
                ...
                valid_traj_masks = valid_traj_masks.to(track_bbox_results[i][0].tensor.device)
                bboxes = track_bbox_results[i][0].tensor[valid_traj_masks]
                bbox_tensor = gt_bboxes_3d[i][-1].tensor
                matched_gt_idx = matched_gt_idx.to(bbox_tensor.device)
