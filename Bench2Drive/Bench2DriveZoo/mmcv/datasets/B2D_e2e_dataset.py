...
class B2D_E2E_Dataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        #     if frame['folder'] != self.data_infos[index]['folder']:
        #         vel_list.append([0.0])
        #         acc_list.append([0.0])
        #         yaw_list.append([0.0])
        #         continue

        #     vel = frame.get('ego_vel', [0.0, 0.0, 0.0])
        #     acc = frame.get('ego_accel', [0.0, 0.0, 0.0])
        #     yaw = frame.get('ego_yaw', 0.0)

        #     vel_scalar = vel[0] if isinstance(vel, (list, np.ndarray)) and len(vel) > 0 else 0.0
        #     acc_scalar = acc[0] if isinstance(acc, (list, np.ndarray)) and len(acc) > 0 else 0.0
        #     yaw_scalar = yaw if isinstance(yaw, (float, int)) else 0.0

        #     vel_list.append([vel_scalar])
        #     acc_list.append([acc_scalar])
        #     yaw_list.append([yaw_scalar])

        # while len(vel_list) < 6:
        #     vel_list.insert(0, [0.0])
        #     acc_list.insert(0, [0.0])
        #     yaw_list.insert(0, [0.0])

        # input_dict['velocity'] = torch.tensor(vel_list, dtype=torch.float32).unsqueeze(0)     
        # input_dict['acceleration'] = torch.tensor(acc_list, dtype=torch.float32).unsqueeze(0)  
        # input_dict['yaw'] = torch.tensor(yaw_list, dtype=torch.float32).unsqueeze(0)   
        input_dict['velocity'] = annos.get('velocity', np.zeros(3))
        input_dict['acceleration'] = annos.get('acceleration', np.zeros(3))
        input_dict['yaw'] = annos.get('yaw', np.zeros(3))
    ...
    def get_ann_info(self, index):
        ...
        anns_results = dict(
            ...
            velocity=self.data_infos[index]['ego_vel'],
            acceleration=self.data_infos[index]['ego_accel'],
            yaw=self.data_infos[index]['ego_yaw'],
            )
        return anns_results
