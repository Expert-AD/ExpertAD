...
TEAM_AGENT=team_code/uniad_b2d_agent_mr.py
# Must set YOUR_CKPT_PATH
TEAM_CONFIG=Bench2DriveZoo/adzoo/uniad/configs/stage2_e2e/base_e2e_b2d.py+Bench2DriveZoo/ckpts/uniad_base_b2d.pth+Bench2DriveZoo/adzoo/bevformer/configs/bevformer/bevformer_base_b2d.py+Bench2DriveZoo/ckpts/bevformer_base_b2d.pth
...
if [ ! -f "${BASE_ROUTES}_${ALGO}_${PLANNER_TYPE}_split_done.flag" ]; then
    ...
    TASK_NUM=2 # 8*H100, 1 task per gpu
...
for ((i=0; i<$length; i++ )); do
    ...
    echo -e "***********************************************************************************"
    bash -e leaderboard/scripts/run_evaluation_MAP.sh $PORT $TM_PORT $IS_BENCH2DRIVE $ROUTES $TEAM_AGENT $TEAM_CONFIG $CHECKPOINT_ENDPOINT $SAVE_PATH $PLANNER_TYPE $GPU_RANK 2>&1 > ${BASE_ROUTES}_${TASK_LIST[$i]}_${ALGO}_${PLANNER_TYPE}.log &
    sleep 5
done
wait