...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path,
                             '{}_infos_temporal_train_ego.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path,
                                 '{}_infos_temporal_val_ego.pkl'.format(info_prefix))
...
def my_fill_trainval_infos(nusc,
                         nusc_can_bus,
                         train_scenes,
                         val_scenes,
                         test=False,
                         max_sweeps=10,
                         fut_ts=6,
                         his_ts=2,ego_pose_json='./data/info/ego_pose_data.json'):
    """Generate the train/val infos from the raw data.
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        sample_ego_pose = ego_pose_data.get(sample['token'], {})
        ##
        info = {
            'lidar_path': lidar_path,
            'token': sample['token'],
