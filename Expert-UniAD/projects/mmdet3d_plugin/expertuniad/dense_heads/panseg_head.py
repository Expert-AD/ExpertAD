...
class PansegformerHead(SegDETRHead):
    ...
    def __init__(
        ...
        self.count = 0
        # self.map_adapter = Adapter(epsilon=0.001, T=100)
        self.map_adapter = build_from_cfg(map_adapter,Adapter)
    ...
    def forward_test(self,
        ...
        pts_feats_map_local, selection_map_scores,top_k_indices = self.map_adapter(pts_feats)
    
        
        pts_feats_expanded =  torch.zeros_like(pts_feats).to(device)
        pts_feats_expanded[:, :, top_k_indices] = pts_feats_map_local.to(device)
        selection_map_scores = selection_map_scores.view(1, 1, 256).to(device)
        pts_feats  =  pts_feats_expanded *  selection_map_scores + pts_feats  
    ...
    def forward_train(self,
        ...
        bev_feat_map_local, selection_map_scores, top_k_indices = self.map_adapter(bev_feat)

        bev_feat_expanded =  torch.zeros_like(bev_feat).to(device)
        bev_feat_expanded[:, :, top_k_indices] = bev_feat_map_local.to(device)
        selection_map_scores = selection_map_scores.view(1, 1, 256).to(device)
        bev_feat  =  bev_feat_expanded *  selection_map_scores + bev_feat  
