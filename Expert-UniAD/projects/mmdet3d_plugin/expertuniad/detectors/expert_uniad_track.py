...
class ExpertUniADTrack(MVXTwoStageDetector):
    """ExpertExpertUniAD tracking part
    """
    def __init__(
        self, 
        use_grid_mask=False,
        ...
        super(ExpertUniADTrack, self).__init__(
        ...
        self.track_adapter = build_from_cfg(track_adapter,Adapter)
    ...
    def _forward_single_frame_train(
        ...
        bev_embed_track_local, selection_track_scores, top_k_indices = self.track_adapter(bev_embed)
        
        bev_expanded =  torch.zeros_like(bev_embed).to(device)
        bev_expanded[:, :, top_k_indices] = bev_embed_track_local.to(device)
        selection_track_scores = selection_track_scores.view(1, 1, 256).to(device)
        bev_embed_track  =  bev_expanded *  selection_track_scores + bev_embed
