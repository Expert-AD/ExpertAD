...
def load_ego_pose_data(json_file):
    """Load ego pose data from a JSON file."""
    with open(json_file, 'r') as f:
        ego_pose_data_dict = json.load(f)
    return ego_pose_data_dict
...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = my_fill_trainval_infos(
    ...
    if test:
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path,
                             '{}_infos_train_ego.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path,
                                 '{}_infos_val_ego.pkl'.format(info_prefix))
...
def my_fill_trainval_infos(nusc, nusc_can_bus, train_scenes, val_scenes, test=False, max_sweeps=10, 
                                                                ego_pose_json ='./data/info/ego_pose_data.json'):
    """Generate the train/val infos from the raw data."""
    ego_pose_data = load_ego_pose_data(ego_pose_json)
    train_nusc_infos = []
    val_nusc_infos = []
    frame_idx = 0
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # Extract ego pose data using sample_token and timestamp
        sample_ego_pose = ego_pose_data.get(sample['token'], {}).get(str(sample['timestamp']), {})
        ...
        info = {
            ...
            'ego2global_rotation': pose_record['rotation'],
            'timestamp': sample['timestamp'],
            'ego_pose_data': sample_ego_pose  # Add ego_pose_data to info
        }
        ...
        if not test:
            ...
            info['valid_flag'] = valid_flag
            info['gt_inds'] = np.array(instance_inds)
            info['gt_ins_tokens'] = np.array([ann['instance_token'] for ann in annotations])
            info['fut_traj'] = future_traj_all
            info['fut_traj_valid_mask'] = future_traj_valid_mask_all
