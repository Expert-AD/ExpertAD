...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        ego_pose_data = info['ego_pose_data']
        velocity_tensor = torch.tensor([item[0] for item in ego_pose_data], dtype=torch.float32)
        acceleration_tensor = torch.tensor([item[1] for item in ego_pose_data], dtype=torch.float32)
        yaw_tensor = torch.tensor([item[2] for item in ego_pose_data], dtype=torch.float32)

        velocity_tensor = velocity_tensor.unsqueeze(1)
        acceleration_tensor = acceleration_tensor.unsqueeze(1)
        yaw_tensor = yaw_tensor.unsqueeze(1)
        ego_pose_data = np.array(ego_pose_data)
        input_dict['velocity'] = velocity_tensor
        input_dict['acceleration'] = acceleration_tensor
        input_dict['yaw'] = yaw_tensor
        return input_dict
