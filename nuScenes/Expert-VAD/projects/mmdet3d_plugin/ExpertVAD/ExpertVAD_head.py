...
from projects.mmdet3d_plugin.ExpertVAD.utils.traj_lr_warmup import get_traj_warmup_loss_weight
from projects.mmdet3d_plugin.ExpertVAD.utils.map_utils import (
...
class MoATransformerInteraction(nn.Module):
    ...
    def __init__(self,
                ...
                norm_cfg=None,
                init_cfg=None):
        super(MoATransformerInteraction, self).__init__()
        
        self.batch_first = batch_first
        self.k = k

        # gating
        self.routing_network = RoutingNetwork(embed_dims, num_experts, k, device)

        self.num_experts = num_experts


    def forward(self, query, track_query, map_query, query_pos, velocity_query, acceleration_query, yaw_query):

        B, A, D = query.shape
        device = query.device
        
        self.routing_network.to(device)
        # Obtain top-k gates and indices from routing network
        top_k_gates, top_k_indices  = self.routing_network(query)  # top_k_indices (B*A, K)

        output = []
        for i in range(self.k):
            expert_idx = top_k_indices[0, i]
            if expert_idx == 0:
                expert_output = track_query
            elif expert_idx == 1:
                expert_output = velocity_query
            elif expert_idx == 2:
                expert_output = acceleration_query
            else:
                expert_output = yaw_query
        ...
        output = torch.cat(
                [output.permute(1, 0, 2),
                 map_query.permute(1, 0, 2)],
                 dim=-1)

        return output

@HEADS.register_module()
class ExpertVADHead(DETRHead):
    """Head of ExpertVAD model.
    Args:
    ...
    def __init__(self,
        ...
        if map_class_weight is not None and (self.__class__ is ExpertVADHead):
        ...
        super(ExpertVADHead, self).__init__(*args, transformer=transformer, **kwargs)
    ...
    def forward(self,
                ...
                ego_his_trajs=None,
                ego_lcf_feat=None,
                velocity=None, acceleration=None,yaw=None
            ):
        """Forward function.
        Args:
        ...
        velocity_conf = outputs_classes[-1]
        velocity_query = motion_hs.reshape(batch, num_agent, -1)
        velocity_query = self.agent_fus_mlp(velocity_query) 
        B, A, D =  velocity_query.shape
        velocity_pos = velocity[:,-1,:].expand(B, A, 2)
        velocity_query, velocity_pos, velocity_mask = self.select_and_pad_query(
            velocity_query, velocity_pos, velocity_conf,
            score_thresh=self.query_thresh, use_fix_pad=self.query_use_fix_pad
        )
        velocity_pos = self.ego_agent_pos_mlp(velocity_pos)

        # ego <-> velocity interaction
        ego_velocity_query = self.ego_agent_decoder(
            query=ego_query.permute(1, 0, 2),
            key=velocity_query.permute(1, 0, 2),
            value=velocity_query.permute(1, 0, 2),
            query_pos=ego_pos_emb.permute(1, 0, 2),
            key_pos=velocity_pos.permute(1, 0, 2),
            key_padding_mask=velocity_mask)

        acceleration_conf = outputs_classes[-1]
        acceleration_query = motion_hs.reshape(batch, num_agent, -1)
        acceleration_query = self.agent_fus_mlp(acceleration_query) 
        B, A, D =  acceleration_query.shape
        acceleration_pos = acceleration[:,-1,:].expand(B, A, 2)
        acceleration_query, acceleration_pos, acceleration_mask = self.select_and_pad_query(
            acceleration_query, acceleration_pos, acceleration_conf,
            score_thresh=self.query_thresh, use_fix_pad=self.query_use_fix_pad
        )
        acceleration_pos = self.ego_agent_pos_mlp(acceleration_pos)

        # ego <-> acceleration interaction
        ego_acceleration_query = self.ego_agent_decoder(
            query=ego_query.permute(1, 0, 2),
            key=acceleration_query.permute(1, 0, 2),
            value=acceleration_query.permute(1, 0, 2),
            query_pos=ego_pos_emb.permute(1, 0, 2),
            key_pos=acceleration_pos.permute(1, 0, 2),
            key_padding_mask=acceleration_mask)

        yaw_conf = outputs_classes[-1]
        yaw_query = motion_hs.reshape(batch, num_agent, -1)
        yaw_query = self.agent_fus_mlp(yaw_query) 
        B, A, D =  yaw_query.shape
        yaw_pos = yaw[:,-1,:].expand(B, A, 2)
        yaw_query, yaw_pos, yaw_mask = self.select_and_pad_query(
            yaw_query, yaw_pos, yaw_conf,
            score_thresh=self.query_thresh, use_fix_pad=self.query_use_fix_pad
        )
        yaw_pos = self.ego_agent_pos_mlp(yaw_pos)

        # ego <-> yaw interaction
        ego_yaw_query = self.ego_agent_decoder(
            query=ego_query.permute(1, 0, 2),
            key=yaw_query.permute(1, 0, 2),
            value=yaw_query.permute(1, 0, 2),
            query_pos=ego_pos_emb.permute(1, 0, 2),
            key_pos=yaw_pos.permute(1, 0, 2),
            key_padding_mask=yaw_mask)
        ...
        self.moa_interaction_layer = MoATransformerInteraction(
        ...
        ego_feats = self.moa_interaction_layer(
                query = ego_map_query,
                track_query = ego_agent_query,
                map_query = ego_map_query,
                query_pos= ego_pos_emb,
                velocity_query = ego_velocity_query,
                acceleration_query = ego_acceleration_query,
                yaw_query = ego_yaw_query,
                )
