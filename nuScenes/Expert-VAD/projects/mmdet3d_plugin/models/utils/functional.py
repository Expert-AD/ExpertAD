...
def features2posemb2d(features, num_pos_feats=256, temperature=10000):
    scale = 2 * math.pi
    features = features * scale
    dim_t = torch.arange(num_pos_feats, dtype=torch.float32, device=features.device)
    dim_t = temperature ** (2 * (dim_t // 2) / num_pos_feats)
    features = features[..., None] / dim_t
    embed = torch.stack((features[..., 0::2].sin(), features[..., 1::2].cos()), dim=-1)
    return embed.view(features.shape[0], features.shape[1], -1)
